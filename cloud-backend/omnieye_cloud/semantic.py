...
def _prompt_for_mode(mode: SemanticMode, query: str | None) -> str:
    ...
    if mode is SemanticMode.SURROUNDINGS:
        return (
            ...
            "这是一张全景图，不要只描述正前方。请按前方、左侧、右侧、后方、头顶、脚下这些方向综合观察，"
            "重点说明各方向是否有通道、货架、路口、车辆、人群、桌椅、门窗、台阶、树枝或其他障碍物。"
            "如果后方或某个方向看不清，要明确说该方向不确定。"
            "summary 必须是一句适合直接中文语音播报的话，优先覆盖前方、后方、左侧、右侧的关键信息，"
            "并给出继续前进、等待、绕行、转身确认或靠近确认的行动建议。"
