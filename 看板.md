...
- 2026-05-27 17:40 左右 UI 实测显示：`避障` 总耗时 72801 ms，其中 `X4 拍照 67090 ms`、云端往返 5711 ms、后端 4841 ms。当前最大瓶颈不是后端，而是 OSC 完整拍照、相机落文件、再下载 5888x2944 全景图。
- 已确认 `D:\MyProject\Bohack2\赛事SDK包（Android+iOS）(2).rar` 内含官方 Android 示例工程 `sdk_demo_1.9.11/sdkdemo2/`，依赖为 `com.arashivision.sdk:sdkcamera:1.9.11` 和 `com.arashivision.sdk:sdkmedia:1.9.11`；后续要用它的预览流/帧提取替代当前 OSC 完整拍照路径。
- `D:\MyProject\Bohack2\赛事SDK包（Windows+Linux）(2)\` 是桌面 CameraSDK/MediaSDK，不适合直接集成进 Android APK。
- SDK 压缩包、SDK assets、SDK 示例源码、`.aar/.so`、模型权重仍然只保留本地，不提交到 Git。后续如接 SDK，优先用 Maven 依赖和本地 Gradle 配置，避免把 SDK 内容上传仓库。
- 短期补丁：`避障` 现在会优先复用 App 内最近一次 X4 bitmap；只有没有最近帧时才重新触发 OSC 拍照。这样第一张图仍慢，但连续分析不会每次都卡在完整拍照。
...
flowchart LR
    ...
    SDK[影石官方 SDK 集成]
    SDKDemo[Android+iOS SDK 包<br/>sdk_demo_1.9.11]
    ...
    SDKDemo -.->|绿线：本地已确认，不进 Git| SDK
    SDK -.->|黄线：下一阶段接预览帧| Android
...
- [x] 已确认本地存在官方 Android+iOS SDK 包，可参考其 `sdkdemo2` Android 示例工程。
- [x] 已确认 Windows/Linux SDK 包不能直接作为 Android APK SDK 使用。
- [x] 已定位当前 `避障` 主要慢点：OSC 完整拍照链路约 67 秒，远高于云端处理耗时。
- [x] `避障` 已改为优先复用最近 X4 bitmap，降低连续点击时的重复拍照等待。
...
- [x] 评估 SDK 包类型：Android+iOS 包内有可用 Android 示例工程，Windows/Linux 包不适合直接进 APK。
- [ ] 基于官方 Android SDK 示例做最小集成方案：连接 X4 WiFi -> 打开 preview stream -> 提取低分辨率帧 -> 转 bitmap 上传。
- [ ] 用 SDK 预览帧替换当前 OSC 完整拍照作为实时避障取帧路径。
...
- [x] 影石 Android+iOS SDK 包已确认在本地根目录，但当前未被 Git 跟踪。
- [ ] 影石 SDK 只本地引用或按合法方式配置，不上传 SDK 内容、SDK 示例源码、SDK assets、`.aar/.so`。
